"""
train_uda_multitask_ablation.py
────────────────────────────────────────────────────────────────────────
消融實驗訓練腳本。原始訓練邏輯不變，透過 AblationConfig 的四個旗標
控制每個 UDA 元件的開關：

    use_fda         : Fourier Domain Augmentation（TS task）
    use_adversarial : Adversarial training（所有 task 的 discriminator）
    use_ema         : EMA Teacher 偽標籤（RM / LL target domain）
    use_classmix    : ClassMix 混合增強（RM；依賴 EMA）

消融設定對應 configs/ablation/ 底下的 JSON：
    source_only.json  → 全關
    plus_fda.json     → FDA only
    plus_adv.json     → FDA + Adv
    plus_ema.json     → FDA + Adv + EMA
    @full_model.json   → 全開（與主腳本等價）

Usage:
    python tools/train_uda_multitask_ablation.py configs/ablation/plus_adv.json
    python tools/train_uda_multitask_ablation.py configs/ablation/plus_adv.json logs/some_checkpoint/latest_model.pth
"""
import sys
import os
import gc
...
def main(cfg: AblationConfig, exp_name: str, checkpoint: str, log_dir: str):

    # ── 印出消融設定 ──────────────────────────────────────────────────
    print("=" * 60)
    print(f"[Ablation] use_fda={cfg.use_fda}  use_adversarial={cfg.use_adversarial}"
          f"  use_ema={cfg.use_ema}  use_classmix={cfg.use_classmix}")
    print("=" * 60)
    ...
    if checkpoint:
        ...
        print(f"  -> 跳過 {n_skip} 個 shape 不相容的 key")
        missing, _ = model.load_state_dict(filtered, strict=False)
        print(f"  -> 模型載入完成。Missing keys: {len(missing)}")
        ...
        if "optimizer_state_dict" in ckpt and "scheduler_state_dict" in ckpt:
            try:
                ...
                start_iter = 0
                print(f"  -> optimizer/scheduler 載入失敗（{e}），Fine-tuning 模式。")
        else:
            start_iter = 0
            print(f"  -> Fine-tuning 模式：checkpoint 無 optimizer 狀態，Iteration 歸零。")
        ...
        for b in range(batch_np.shape[0]):
            ...
            for lbl in range(1, num_labels):
                ...
                try:
                    ...
                    thresh = 15.0 if len(ys) < 50 else 30.0
                    if np.mean(np.abs(xs - xs_fit)) >= thresh:
                        continue
                    filtered[ys, xs] = 1
    ...
    def run_task_step(task_name, dl_src, dl_tgt, classmix, categories, _palette, metric, iter_idx):
        ...
        if task_name in ["ts", "tl"]:
            ...
            # [ABLATION] FDA：只對 TS 且 use_fda=True 時套用
            if task_name == "ts" and cfg.use_fda:
            ...
            # [ABLATION] Adversarial：只在 use_adversarial=True 時進入
            _ADV_INTERVAL = 2
            if dl_tgt is not None and cfg.use_adversarial and iter_idx % _ADV_INTERVAL == 0:
        ...
        # ── Case B: 車道線 (LL) ───────────────────────────────────────
            ...
            # [ABLATION] Target domain block：EMA 或 Adversarial 至少一個開啟才進入
            do_target = (cfg.use_ema or cfg.use_adversarial) and dl_tgt is not None and iter_idx % ema.update_every == 0
            if do_target:
                tgt_data     = next(dl_tgt)
                tgt_imgs     = tgt_data["img"].to(device)
                WARMUP_ITERS = 1500
                if iter_idx > WARMUP_ITERS:
                    ...
                    if cfg.use_ema:
                        ...
                        # EMA 關閉：仍需計算 latent 供 adversarial 使用
                        with torch.amp.autocast("cuda", enabled=cfg.autocast, dtype=torch.bfloat16):
                            latent_tgt = model.forward_encoder(tgt_imgs)
        ...
        # ── Case C: Road Marking (RM) ─────────────────────────────────
            ...
            with torch.amp.autocast("cuda", enabled=cfg.autocast, dtype=torch.bfloat16):
                ...
                if boundary_small is not None:
                    boundary_pred_full = F.interpolate(
                        ...
                        size=boundary_gt.shape[-2:], mode='bilinear', align_corners=False
                    ).squeeze(1)
            ...
            # [ABLATION] Target domain block：EMA 或 Adversarial 至少一個開啟才進入
            WARMUP_ITERS = 1500
            do_target = (cfg.use_ema or cfg.use_adversarial) and iter_idx % ema.update_every == 0 and iter_idx > WARMUP_ITERS
            if do_target:
                tgt_data            = next(dl_tgt)
                ...
                latent_tgt       = None
                tgt_imgs_fwd_dev = None  # target images forwarded through model (for adversarial)

                # [ABLATION] EMA 偽標籤 + ClassMix
                if cfg.use_ema:
                    ema.ema_model.eval()
                    with torch.no_grad(), torch.amp.autocast("cuda", enabled=cfg.autocast, dtype=torch.bfloat16):
                    ...
                    # [ABLATION] ClassMix：use_classmix=True 才混合
                    if cfg.use_classmix:
                        classmix.mix(
                    ...
                    with torch.amp.autocast("cuda", enabled=cfg.autocast, dtype=torch.bfloat16):
                        ...
                        if cfg.use_classmix and erased_tgt_imgs_cpu is not None:
                    ...
                    tgt_imgs_fwd_dev = tgt_imgs_mixed_dev

                else:
                    # EMA 關閉：取得 target latent 供 adversarial 使用
                    with torch.amp.autocast("cuda", enabled=cfg.autocast, dtype=torch.bfloat16):
                        tgt_imgs_dev = [im.to(device) for im in tgt_imgs_cpu]
                        latent_tgt   = model.forward_encoder(tgt_imgs_dev[tgt_view_idx])
                    tgt_imgs_fwd_dev = tgt_imgs_dev
                ...
                if cfg.use_adversarial:
                    ...
                    with torch.amp.autocast("cuda", enabled=cfg.autocast, dtype=torch.bfloat16):
                        ...
                        loss_d_tgt  = compute_domain_discrimination_loss_mt(
                            model, tgt_imgs_fwd_dev, None, tgt_dis_lbl, None, cfg.crop_size, task_name, device,
                            latent=latent_tgt,
                        )
                        if not (torch.isnan(loss_d_tgt) or torch.isinf(loss_d_tgt)):
                            disc_loss += loss_d_tgt * 0.2
    ...
    for iter_idx in range(start_iter + 1, cfg.max_iters + 1):
        ...
        task_weights = cfg.task_weight if isinstance(cfg.task_weight, dict) else vars(cfg.task_weight)
        ll_interval  = cfg.ll_interval
        ts_interval  = cfg.ts_interval
        tl_interval  = cfg.tl_interval
    ...
    print(f"Peak VRAM Allocated : {max_mem_allocated:.2f} GB")
    print(f"Peak VRAM Reserved  : {max_mem_reserved:.2f} GB")
    print("=" * 30 + "\n")
    writer.close()


if __name__ == "__main__":
    assert len(sys.argv) >= 2, "Usage: python train_uda_multitask_ablation.py <config.json> [checkpoint.pth]"
    cfg        = AblationConfig.load(sys.argv[1])
